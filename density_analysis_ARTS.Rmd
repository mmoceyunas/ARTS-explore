...
Estimate bandwidth
```{r}
sigma_value = bw.diggle(u_c_ppp_km) #suggested if detecting pattern of predominantly tight clusters
plot(sigma_value)
```

```{r}
sigma_value = bw.ppl(u_c_ppp_km) #suggested if detecting pattern of predominantly tight clusters
plot(sigma_value)
```
...
```
=======
K1 <- density(u_c_ppp_km, eps = 1, sigma = sigma_value)
plot(K1, main='RTS density in Nitze bounds', las=1)
contour(K1, add=TRUE)
```
>>>>>>> a3a8e7d647c92a47b00e999f7b82059082ecbb82

### Covariate Adjusted

```{r prep PCA 1}
pca <- rast("../rts_rep_analysis/pca/pca.tif")
