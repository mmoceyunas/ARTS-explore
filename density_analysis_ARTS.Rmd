...
nitze_bbox_2 = bboxes |> #example slice
  st_remove_holes() |>
  st_union() |>
  st_cast('POLYGON') |>
  st_as_sf() |>
...
arts_bboxes_2 = arts_sf |> #example slice
  filter(
    map_lgl(
...
```{r prep PCA 1+2}
band1 <- subset(pca_r, 1)
band2 <- subset(pca_r, 2)
band12 <- c(band1, band2)
...
```{r prep PCA 1+2}
band3 <- subset(pca_r, 3)
band5 <- subset(pca_r, 5)
band35 <- c(band3, band5)
...
## Covariate Adjusted - PCA2 + PCA3 + PCA5
```{r prep PCA 1+2}
band3 <- subset(pca_r, 3)
band4 <- subset(pca_r, 4)
band5 <- subset(pca_r, 5)
band345 <- c(band3, band5, band4)
