...
=======
>>>>>>> de1e0f0cd4a5982ca6c9d4de1ef30f81e6cf4c7c
```{r null model}
...
arts_p1_sf = arts_sf |>
  filter(
    map_lgl(
      st_intersects(arts_sf, p1_sf, sparse = TRUE), 
      ~ length(.x) > 0
    )
  )
...
arts_p2_sf = arts_sf |>
  filter( 
    map_lgl( 
      st_intersects(arts_sf, p2_sf, sparse = TRUE), 
      ~ length(.x) > 0 
    )
  )
...
arts_p3_sf = arts_sf |>
  filter( 
    map_lgl( 
      st_intersects(arts_sf, p3_sf, sparse = TRUE), 
      ~ length(.x) > 0 
    )
  )
...
ann.p <- mean(nndist(centroids_p3_ppp, k=1))
ann.p
```
```{r Null simulation}
n     <- 500L               # Number of simulations
